...
$Log: init.fun,v $
Revision 1.6  1996/03/12  22:26:48  esb
added loopback interface to IP setup.
...
*)
	 ...
	 fun getname () =
	      ((get_quoted_field (Read.findline (rc_config,
	       ...
	       handle _ => get_quoted_field (Read.findline
					     (rc_config, "HOSTNAME=")))
	in
	 fun local_hostname () = protect ("Ip", getname)
	end
	...
	fun setup () =
	  let
	    ...
	    val loopback_ip = Word32.+ (Word32.<< (Word32.fromInt 0x7f, 0w24),
					Word32.fromInt 1)
	    val loopback = {local_id = loopback_ip, interface = "lo0",
